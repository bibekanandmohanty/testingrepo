        ...
        <figure>
            <div class="product-image product-image-zoom" style=" position: relative;display: block;border: 1px solid #ededed;background: #fff;">
                <div style="z-index: 0;font-size: 26px;position: relative;">                
                    ...
                    <img class="js-modal-product-cover product-cover-modal" width="{$products.cover.large.width}" src="{$products.cover.large.url}" alt="{$products.cover.legend}" title="{$products.cover.legend}" itemprop="image"> 
                </div>
            </div>
            <figcaption class="image-caption">
              {block name='product_description_short'}
                <div id="product-description-short" itemprop="description">{$products.description_short nofilter}</div>
              {/block}
            </figcaption>
        </figure>
              ...
              <ul class="product-images js-modal-product-images">
                {foreach from=$products.images item=image}
                  <li class="thumb-container" style="display: inline;position: relative; vertical-align: top;">
                    <img  onclick="changePredecoImage({if !($products.is_default_attribute)}'{$image.bySize.home_default.predecourl}'{/if})"
