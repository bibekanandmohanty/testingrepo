...
class ProductLazyArray extends AbstractLazyArray
{
    ...
    private function fillImages(
        ...
        if ((bool) Configuration::get('PS_SSL_ENABLED') && $custom_ssl_var == 1) {
            $xe_store_url = _PS_BASE_URL_SSL_ . __PS_BASE_URI__;
        } else {
            $xe_store_url = _PS_BASE_URL_ . __PS_BASE_URI__;
        }
        ...
        if (isset($this->product['ref_id']) && $this->product['ref_id'] > 0) {
            if ((bool) Configuration::get('PS_SSL_ENABLED')) {
                $xe_store_url = _PS_BASE_URL_SSL_ . __PS_BASE_URI__;
            } else {
                $xe_store_url = _PS_BASE_URL_ . __PS_BASE_URI__;
            }
            $pid = $this->product['id_product_attribute'] ? $this->product['id_product_attribute'] : $this->product['id_product'];
            $url_ref = $xe_store_url . $xetoolDir . '/api/v1/preview-images?custom_design_id=' . $this->product['ref_id'] . '&product_id=' . $pid;
            $results = file_get_contents($url_ref);
            $predeco_image = json_decode($results, true);
            $img_default = '';
            foreach ($predeco_image[$this->product['ref_id']] as $key => $v) {
                if ($v['design_status'] == '1') {
                    $img_default = $v['customImageUrl'][$key];
                }
                if ($v['display_edit'] == '1') {
                    $this->product['xe_is_edit'] = 1;
                }
                if ($v['nameAndNumber'] == '1') {
                    $this->product['xe_is_nameandnumber'] = 1;
                }
                if (isset($v['variableDecorationSize']) && $v['variableDecorationSize'] != '' && !is_null($v['variableDecorationSize'])) {
                    $this->product['is_custom_dimension'] = true;
                    $this->product['custom_dimension'] = $v['variableDecorationSize'] . ' ' . $v['variableDecorationUnit'];
                } else {
                    $this->product['is_custom_dimension'] = false;
                }
                $this->product['xe_printid'] = $v['printid'];
            }
            $this->product['cover']['bySize']['cart_default']['url'] = $img_default;
        }
        //ImprintNext custom code end
    }
    /**
    ...
    public function getVariantIdByPid($pid, $default_attribute_id)
    {
        ...
        if (!empty($row_pre)) {
            ...
            $imageRetrievers = new ImageRetriever($this->link);
